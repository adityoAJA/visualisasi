...
with tabs[1]:
    ...
    if uploaded_file is not None:
        ...
        def load_data(file_content):
            with xr.open_dataset(io.BytesIO(file_content), engine='h5netcdf', decode_times=False) as data:
                return data.load()
    
        # Load the data
        data = load_data(file_content)
    
        # Ensure 'time' coordinate is converted to a valid DatetimeIndex
        data['time'] = pd.to_datetime(data['time'].values, unit='D', origin=pd.Timestamp('1900-01-01'))
        ...
        df = pd.DataFrame({
                'Rentang Waktu': data['time'].values,
